...
static int crypto_chrdev_open(struct inode *inode, struct file *filp)
{
	...
	*host_fd = -1;
	...
	//if (down_interruptible(&crdev->sema))
 	//	return -ERESTARTSYS;
...
int i=0;
	...
	*syscall_type = VIRTIO_CRYPTO_SYSCALL_IOCTL;
	...
	sgs[num_out++] = &host_sg;
        printk("To cmd einai %d \n" ,*cmd_ad);
        //printk("To string cmd einai %s \n",*cmd_ad);

        switch (cmd) {

        case CIOCGSESSION: *cmd_ad=1;
                            break;
 
	case CIOCFSESSION:  *cmd_ad=2;
                            break;

	case CIOCCRYPT:    *cmd_ad=3;
                            break;
	...
	case CIOCCRYPT:
		debug("CIOCCRYPT");
                ...
                printk("To len einai %d\n" , crypt->len);
                printk("To iv einai = %s\n", iv);
                printk("To src einai = %s\n",src);
		printk("To dest pano einai %s \n" ,crypt->dst);
                dest = crypt->dst;
	...
	//up(&crdev->sema);
        ...
        switch(rem){
        case 1:
                  //printk("To session prin tin copy to user einai %s \n" ,session);
        ...
        case 2:
                  ...
                  break;
        case 3:
                  printk("To dest prin tin copy to user einai %s \n" ,dest);
                  printk("To dst prin tin copy to user einai %s \n" ,dst);
