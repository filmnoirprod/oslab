...
int main(void)
{
	...
	struct sockaddr_in sa;
        ...
        //set of socket descriptors we want to read waking up with select
        fd_set readfds;
	...
	for (;;) {
		...
		for (;;) {
                    ...
                    if(select(newsd + 1 , &readfds , NULL , NULL , NULL) < 0){
                        perror("select");
                        exit(1);
                    }
			...
			for (i=0; i< DATA_SIZE; i++){
                        ...
                        fprintf(stdout, "\nFriend:\n");
                        fflush(stdout);                        
			if (insist_write(0, decrypted, i) != i) {
